...
do {    
    ...
    console.log(`!!!!Congratulations, you earned ${pts}pt(s)!!!!`);
    let max = maxPointsCalc(currentLevel);
    percentifyMe(max, pts);
...
//asks the first question to the user to start the game, returns string attend or string skip
   ...
   function maxPointsCalc(levelNum) {
       let halfway = 0;
       let nearly = 0;
        for (let i = 1; i <= levelNum; i++) {
            halfway = (levelNum - i) + 2;
   ...
   //function calculates the percentage of heroic-ness
    ...
    {
        percentify = ((pts)/(max)) * 100;
        return console.log(`You are ${Math.round(percentify)}% heroic\n\n`);
    }